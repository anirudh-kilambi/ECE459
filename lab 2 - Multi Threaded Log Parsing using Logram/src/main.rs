...
fn main() {
    ...
    // need to take care of the num-threads arg and single_map arg
    let num_threads = args.num_threads.unwrap();
    let single_map = match args.single_map {
        None => false,
        Some(sm) => {
            if sm {
                true
    ...
    let (double_dict, triple_dict, _all_token_list) =
        packages::parser::parse_raw(input_fn.unwrap(), &log_format, single_map, num_threads);
